...
function getClient(providerName) {
  ...
  return { client: clientCache[cacheKey], defaultModel: cfg.defaultModel, name };
}
...
async function callOpenAIResponses({ prompt, maxItems, provider, model, timeoutMs = 35 * 60 * 1000 }) {
...
async function callOpenAIResponsesRaw({
  ...
  const response = await client.responses.create(effectivePayload, { timeout: timeoutMs });
