...
/**
 * Проставить идентификаторы продуктам, у которых их нет.
 *
 * Возвращает новый список узлов — исходный не трогает. Если проставлять
 * нечего, возвращает тот же массив: объединение графов сравнивает списки по
 * ссылке, и лишняя копия сбивала бы это сравнение.
 */
export async function resolveProductIds(
  ...
  for (const n of nodes) {
    ...
    if (readProductId(n.data)) continue;
  ...
  const next = nodes.map((n) => {
    ...
    if (readProductId(n.data)) return n;
    const label = typeof n.data?.label === "string" ? n.data.label.trim() : "";
