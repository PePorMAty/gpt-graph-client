...
/**
 * Плотная раскладка окрестности фокус-режима.
 *
 * Камера вписывает всю окрестность в экран, поэтому каждый лишний пиксель
 * между рангами прямо уменьшает масштаб — а с ним и подписи на узлах: на 2–3
 * уровнях текст переставал читаться. Ранги ставим вплотную, а nodeHeight
 * берём близким к реальной высоте узла (~50px): просторные 80px полного графа
 * раздвигали ранги сильнее, чем сам зазор.
 *
 * По ширине запас есть (окрестность вытянута по вертикали), поэтому узел здесь
 * шире обычного — длинные названия реже переносятся на вторую строку.
 * Согласовано с COMPACT_FOCUS_BOX в components/nodes/nodeBox.ts.
 */
export const FOCUS_COMPACT_SPACING = {
  nodeWidth: 260,
  nodeHeight: 52,
  rankSep: 48,
  nodeSep: 56,
