...
export function levelToFlow(levelChain: TechChain, opts: Opts) {
  ...
  // edge: target <-> transformation
  // source = верхний узел, target = нижний → всегда source.Bottom → target.Top
  edges.push({
  ...
  uniqueInputs.forEach((pid, idx) => {
    ...
    // edge: tr <-> input product
    // source = верхний узел → source.Bottom → target.Top
    edges.push({
