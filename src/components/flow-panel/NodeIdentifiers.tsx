          ...
          {okpd2 && (
            <div className={styles.idPopRow}>
              ...
              <span className={styles.idPopValue}>
                ...
                {industry?.okpd2Name && (
                  // Сервер говорит, чьё это название. Точное — значит,
                  // классификатор знает сам код и подпись даём как есть; иначе
                  // название нашлось по укороченному коду, то есть это ГРУППА,
                  // в которую код попал, — так и пишем, чтобы не выдавать
                  // широкую категорию за саму позицию.
                  <span className={styles.idPopNote}>
                    {industry.okpd2NameExact ? "" : "группа: "}
                    {industry.okpd2Name}
                  </span>
                )}
              </span>
