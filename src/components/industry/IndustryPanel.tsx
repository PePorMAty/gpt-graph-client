...
export const IndustryPanel: FC<Props> = ({ productName }) => {
  ...
  // Узел ещё не назван. Реестр про заготовку не спрашивали, поэтому «записи
  // нет» здесь было бы неправдой — не искали. И объяснение про охват реестра
  // тут ни при чём: человеку надо просто вписать название.
  if (info.placeholder) {
    return (
      <div className={styles.empty}>
        <IndustryDataIcon size={30} className={styles.emptyIcon} />
        ...
        <p className={styles.emptyText}>
          На узле стоит подпись-заготовка «{name}». Впишите название вещества —
          и мы поищем его в реестре ПП №719.
        </p>
      </div>
    );
  }
